...
- **Time check (loser/dead-money only — do NOT apply to winners):** This is for cutting dead money, not winners. A green position with structure intact (holding a higher-low) always gets DAYS to work — don't cut winners on a timer.
- **CONFIRMATION DEADLINE (learned the hard way from the XLF + XLE losses — both bled while never confirming):** When your thesis depends on a TRIGGER (a breakout, breakdown, or MA reclaim that has to actually happen), that trigger must FIRE within ~2–3 runs. If instead the name just **COILS / grinds without the break confirming** — it never triggered = dead money = **CUT it now, do not wait for the catastrophic stop.** "Coiling on support/resistance for 3+ runs without breaking" is THE dead-money trap (XLE coiled the $53.40 shelf 6+ runs and bled −14%). This is the opposite of a confirmed winner breathing — distinguish: *confirmed + green + structure intact = hold; unconfirmed + red + just grinding = cut.*
...
- CUT LOSS — cut when the THESIS is broken, regardless of current P&L. Thesis-broken means ANY of: chart structure breaks, catalyst reverses, the trigger fails to confirm within ~2–3 runs (see Confirmation Deadline), OR **price loses the level the thesis was built on ("lost the thesis line") — e.g. a call's support reclaim level or a put's breakdown level.** XLF *lost its $54 line and was held 2 days into a −21% loss* — that's the exact failure mode: a broken thesis line is a CUT, not an overnight hold. The broker catastrophic stop (~55–65% options / ~7–8% equity) is the last-resort floor — don't ride a broken thesis all the way down to it. Still don't panic-cut a CONFIRMED winner on normal noise. Never average down.
...
18. **★ RSI BULLISH DIVERGENCE reversal (strong, high-conviction)** — price prints a lower low while RSI prints a higher low (momentum exhausting to the downside), THEN a confirmation fires: a reversal candle (hammer/engulfing) on volume, or a reclaim of the intervening swing high / 20 EMA. The divergence is the strong tell; the trigger is the entry. Best at major support / oversold. This is a CALL setup.
...
10. **★ RSI BEARISH DIVERGENCE reversal (strong, high-conviction)** — price prints a higher high while RSI prints a lower high (momentum exhausting to the upside), THEN a confirmation fires: a reversal candle (shooting-star/bearish engulfing) on volume, or a break of the intervening swing low / 20 EMA. The divergence is the strong tell; the trigger is the entry. Best at major resistance / overbought. This is a PUT setup — and it's the disciplined way to play an EXTENDED runner rolling over (vs. blindly shorting strength).
...
3. **RSI(14)** from closes — 50–70 rising = healthy; >70 = extended; <30 = oversold.
   - **RSI DIVERGENCE — a STRONG reversal signal, compute it every time (this is a primary edge, not an afterthought):** compare the last two price pivots to RSI at those same bars.
     - **Bullish divergence:** price makes a LOWER low but RSI makes a HIGHER low → selling momentum exhausting → reversal-up setup. Strongest when RSI lows are oversold (~<35) and/or at major support.
     - **Bearish divergence:** price makes a HIGHER high but RSI makes a LOWER high → buying momentum exhausting → reversal-down setup. Strongest when RSI highs are overbought (~>65) and/or at major resistance.
     - Pivots should be ~5–30 bars apart (not adjacent). Check both daily and the 30-min intraday series.
   - **How to trade it (strong, but NOT naked):** divergence makes a reversal setup high-conviction, BUT you still need a LIGHT confirmation trigger to enter — a reversal candle (hammer/engulfing) on volume, OR a break of the intervening swing / MA reclaim. Divergence + that trigger = a top-tier reversal trade (see the "RSI divergence reversal" setup). NEVER enter on divergence alone with no trigger — divergences can persist for a long time in a strong trend (don't short a melt-up just because RSI diverges).
...
**★ AVOID low-ATR sector-ETF directional OPTIONS (learned from the XLF + XLE losses — both were sector-ETF options, both lost).** Sector ETFs (XLF, XLE, XLV, XLK, etc.) have small ATR — the directional move is too small for the option to overcome theta + bid/ask, so they turn into dead-money grinds even when your directional read is right. Now that the account affords single-name options (~$3K+ BP), **trade the leading/lagging STOCK within the sector, not the sector ETF** — single names have real ATR and real edge. Use the sector ETFs only as a *tell* for rotation/regime (Step 3.0), not as the trade vehicle. (The old "use ETFs at small BP" guidance was for a sub-$1K account to avoid single-name premium blowups; you've outgrown it.) If you ever do trade a low-ATR ETF option, demand fast confirmation and cut on the Confirmation Deadline.
...
- **A+ setup** (clean TA + strong regime + at least TWO of: fresh hard catalyst, **aligned options flow (3c+)**, **confirmed RSI divergence**, leading sector): equity up to 60–80% of BP; **single option capped at ~40% of BP** (if you want more A+ exposure, you've maxed the single-option cap — that's intentional). A confirmed divergence reversal + aligned flow is a signature A+ "everything lines up" trade.
- **A setup** (clean TA + at least ONE of: catalyst, aligned options flow, **confirmed RSI divergence**, sector tailwind): equity 40–60%; single option ~25–40%. A divergence reversal with its confirmation trigger fired qualifies here on its own.
